...
with open(config_res.paths['resnet_params_path']) as f_in:
    ...
    class DepthNet:
        ...
        def Net(self):
            out = self.layer_zero(self.input_y)
        ...
        def layer_zero(self, layer_input):
            ...
            with tf.variable_scope('layer0'):
                x = conv2d_init(layer_input, [shapex[0], shapex[1], 1, shapex[3]/2], name = "W", to_train = True)
                x = batchnorm_init(x, self.phase, "0")
                x = tf.nn.relu(x)
        ...
        def normal_res_block(self, layer_input, layer_no, block_no):
            ...
            with tf.variable_scope("normal_res_%d"%layer_no + "_block_%d"%block_no):
                ...
                x = conv2d_init(layer_input, [shapex1[0], shapex1[1], shapex1[2]/2, shapex1[3]/2], name = "W1", to_train = True)
                x = batchnorm_init(x, self.phase, "1")
                x = tf.nn.relu(x)
                x = conv2d_init(x, [shapex2[0], shapex2[1], shapex2[2]/2, shapex2[3]/2], name = "W2", to_train = True)
                x = batchnorm_init(x, self.phase, "2")
                x = residual + x
        ...
        def shortcut_res_block(self, layer_input, layer_no, block_no):
            ...
            with tf.variable_scope("shortcut_res_%d"%layer_no + "_block_%d"%block_no):
                ...
                residual = conv2d_init(layer_input, [shapedn[0], shapedn[1], shapedn[2]/2, shapedn[3]/2], name = "Wdn", to_train = True, stride=[1,2,2,1])
                residual = batchnorm_init(residual, self.phase, "res")

                x = conv2d_init(layer_input, [shapex1[0], shapex1[1], shapex1[2]/2, shapex1[3]/2], name = "W1", to_train = True, stride = [1,2,2,1])
                x = batchnorm_init(x, self.phase, "1")
                x = tf.nn.relu(x)
                x = conv2d_init(x, [shapex2[0], shapex2[1], shapex2[2]/2, shapex2[3]/2], name = "W2", to_train = True)
                x = batchnorm_init(x, self.phase, "2")
